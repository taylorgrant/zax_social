...
monthly_performance <- function(current_month) {
  ...
  thresholds <- tibble(
    network = c("Facebook", "Instagram Post", "Instagram Story", "TikTok", "X"),
  ...
  previous_year <- lubridate::year(Sys.Date()) -
    ifelse(current_month %in% c(12, 1), 1, 0)
  current_year <- lubridate::year(Sys.Date()) -
    ifelse(current_month %in% c(12), 1, 0)

  previous_data <- glue::glue(here::here(
    "data",
    "zaxby_post_{previous_month}.{previous_year}.csv"
  ))
  current_data <- glue::glue(here::here(
    "data",
    "zaxby_post_{current_month}.{current_year}.csv"
  ))

  if (!file.exists(current_data)) {
    stop(glue("Missing current data file: {current_data}"))
  }
  if (!file.exists(previous_data)) {
    stop(glue("Missing previous data file: {previous_data}"))
  }

  # 1. READ IN DATA ----------------------------------------------------------
  previous <- read_csv(previous_data) |>
    janitor::clean_names() |>
    dplyr::filter(!is.na(engagements)) |> # filter out empty engagements (1 post)
    dplyr::filter(!network %in% c("LinkedIn", "YouTube", "Threads")) |>
    dplyr::filter(post_type != "'@Reply") |>
    dplyr::mutate(
    ...
    dplyr::filter(!(network == "X" & post_type == "Quote")) |>
    dplyr::filter(paid_impressions == 0) |>
    dplyr::filter(impressions > 100) |> # to knock out random posts
    dplyr::mutate(date2 = as.Date(date, format = "%m/%d/%Y")) |>
    dplyr::arrange(date2)

  current <- readr::read_csv(current_data) |>
    janitor::clean_names() |>
    dplyr::filter(!is.na(engagements)) |> # filter out empty engagements (1 post)
    dplyr::filter(!network %in% c("LinkedIn", "YouTube", "Threads")) |>
    dplyr::filter(post_type != "'@Reply") |>
    ...
    dplyr::mutate(date2 = as.Date(date, format = "%m/%d/%Y")) |>
    dplyr::arrange(date2) |>
    dplyr::mutate(month = lubridate::floor_date(date2, "month")) |>
    dplyr::select(-date2)
  ...
  previous_totals <- previous |>
    ...
    dplyr::mutate(
      imp = dplyr::case_when(network == "TikTok" ~ video_views, TRUE ~ imp),
      month = lubridate::floor_date(as.Date(date, format = "%m/%d/%Y"), "month")
    ) |>
    dplyr::summarise(
      month = unique(month),
      post_count = dplyr::n(),
  ...
  monthly_totals <- rbind(previous_totals, current_totals) |>
    ...
    dplyr::filter(month == current_totals$month) |>
    dplyr::mutate(dplyr::across(
      dplyr::matches("ER|MOM"),
      ~ round(., digits = 3)
    )) |>
    dplyr::relocate(network, .before = month) %>%
    purrr::set_names(nm = toupper(names(.)))
  ...
  previous_platform_performance <- previous |>
    ...
    dplyr::summarise(
      prev_impressions = mean(imp, na.rm = TRUE),
  ...
  platform_performance <- current |>
    ...
    dplyr::left_join(previous_platform_performance) |>
    dplyr::left_join(dplyr::select(benchmarks$bm_imp, -n), by = "network") |>
    dplyr::left_join(dplyr::select(benchmarks$bm_er, -n), by = "network") |>
    dplyr::mutate(
      mom_imp_delta = round(
        (avg_impressions - prev_impressions) / prev_impressions,
        3
      ),
  ...
  platform_thresholds <- current |>
    get_organic_impressions() |>
    mutate(
      imp = case_when(network == "TikTok" ~ video_views, TRUE ~ imp),
      er = engagements / imp,
    ...
    mutate(
      imp_below = ifelse(imp < imp_p_5, 1, 0),
  ...
  top_bottom_n <- function(data, metric, n = 2) {
    if (nrow(data) == 0) {
      return(data)
    } # <-- early return if group is empty
    ...
    bind_rows(best, worst) %>%
      mutate(
        rating = rep(c("Best", "Best", "Worst", "Worst"), length.out = nrow(.))
      )
  }
  ...
  bw_imp <- current |>
    get_organic_impressions() |>
    mutate(
      imp = if_else(network == "TikTok", video_views, imp),
  ...
  id <- "1IZqRhXTn3UPQbZ-38RmiJlZ2XvWm82vBY6R5Mt8iV8c"
  ...
  googlesheets4::sheet_append(
    tmp$`Instagram Post`,
    ss = id,
    sheet = "Instagram Post"
  )
  googlesheets4::sheet_append(
    tmp$`Instagram Story`,
    ss = id,
    sheet = "Instagram Story"
  )
  ...
  bucket_plot <- function(data) {
    ...
    bucket_counts <- platform_thresholds |>
      select(month:er_t3) |>
      pivot_longer(cols = -c(month, network), values_to = "count")

    bucket_pct <- platform_thresholds |>
      select(month, network, imp_below_pct:er_t3_pct) |>
      pivot_longer(cols = -c(month, network), values_to = "percent") |>
    ...
    imp_bucket_plot <- ggplot(
      data = filter(bucket_full, str_detect(name, "^imp")),
      aes(
        x = network,
        y = percent,
    ...
    ggsave(
      glue::glue(here::here(
        "figures",
        "impression_bucket_{format(unique(bucket_full$month), '%b-%Y')}.png"
      )),
      imp_bucket_plot,
      width = 6.4,
      height = 4.21
...
plot_benchmark_graphs <- function(
  sheet_id = "1IZqRhXTn3UPQbZ-38RmiJlZ2XvWm82vBY6R5Mt8iV8c"
) {
  ...
  all_sheets <- map(
    sheet_names,
    ~ read_sheet(ss = id, sheet = .x, skip = 1) |>
      clean_names()
  )
